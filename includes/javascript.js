  // Create floating letters
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            letterElements.forEach((letterObj, index) => {
                ...
                if (distance < 20) {
                    const angle = Math.atan2(mouseY - letterY, mouseX - letterX) * (180 / Math.PI);
            ...
            letterElements.forEach((letterObj, index) => {
                ...
                if (!letter.classList.contains('active')) {
                    letter.style.transform = `
                        ...
                        rotate(${letterObj.originalRotation + Math.sin(time + index) * 5}deg)
                    `;
                }
            });
        ...
        setInterval(() => {
            letterElements.forEach(letterObj => {
                if (Math.random() < 0.1) { // 10% chance to change letter
                    letterObj.element.textContent = letters[Math.floor(Math.random() * letters.length)];
                }
            });
        }, 3000);
        class TypeWriter {
